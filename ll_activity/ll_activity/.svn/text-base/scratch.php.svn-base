...
$flag=$manager->verifyActivityTime();
if($flag==1){
	$response['code']=ErrorCode::Activity_Scratch_Not_Start;
	$response['err_msg']=ErrorCode::getTaskError($response['code']);
	echo json_encode($response);
	exit;
}else if($flag==2){
    $response['code']=  ErrorCode::Activity_Scratch_Had_End;
	$response['err_msg']=ErrorCode::getTaskError($response['code']);
	echo json_encode($response);
	exit;
}

$result = $manager->getUserScratch($uin,$login_key, $uuid, $productID, $platform, $appid,$range);
