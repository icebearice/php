...
require_once SYSDIR_UTILS . "/LLActivityShareManager.php";
...
if (!$auth->checkUserLogin($productID, $uuid, $platform, $uin, $login_key,$appid)) {
			...
			$response['data']['share_flag']=1;
			echo json_encode($response);
			 exit();
}
//$uin = 3000;
$addCredit=10;
$handler = new LLActivityShareManager();
$response['data']['share_flag'] = $handler->addShareTask($uin,$addCredit);
echo json_encode($response);
