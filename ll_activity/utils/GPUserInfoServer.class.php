...
class GPUserInfoServer {
    ...
    private function getThriftIPProt() {
        $res = $this->etcd_obj->getServer( '/gp_user_content' );  
        if( !$res ) {
    ...
    public function getUser($uid) {
        try {
            $req = new \GPUser\GetUserReq();
            $req->uid = $uid;
            $res = $this->__client->GetUser($req);
            if (!$res->success) {
                return array();
            }
            $base = json_decode(json_encode($res->user->base), true);
            $ex = json_decode(json_encode($res->user->ex), true);
            return array_merge($base, $ex);
        } catch (TEXception $tx) {
            return array();
        }
    }
...
//$obj = new GPUserInfoServer();
//var_dump($obj->getUser(1806727));
