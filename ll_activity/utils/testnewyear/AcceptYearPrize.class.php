...
class AcceptYearPrize{
	...
	public function getUserAcceptPrizeId($uin){  //获取用户今日已领取年货id
		...
		$sql = "select prize_id from ll_activity_user_accept_prize_info where uin = {$uin}";
		$sql.= " and prize_id between 1 and 6 and unix_timestamp(accept_time) >= {$today}";
	...
	public function getYearPrize($uin,$prizeId){  //用户领取年货
		...
		$sql.= " values({$uin},'{$prizeId}','{$prize_name}','{$prize_sour}','{$end_time}')";
		$this->__db->query($sql);
